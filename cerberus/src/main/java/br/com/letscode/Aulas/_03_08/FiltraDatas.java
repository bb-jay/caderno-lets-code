package br.com.letscode.Aulas._03_08;
 ...
 *         aplicá-lo nos lançamentos de seu extrato.
 *         O cliente espera no mínimo seja entregue três filtro conforme abaixo:
 *         Lançamentos até 45 dias.
 *         Lançamentos até 90 dias.
 *         Lançamentos entre <período inicial> e <peródo final>. Se o período
 *         for maior que 90 dias deve lançar uma exception com a seguinte
 *         mensagem “Período deve ser menor que 90 dias.”.
 *         Lançamentos do mês.
 *         Lançamentos do ano.
 *
 *
...
public class FiltraDatas {

	private static final LocalDateTime HOJE = LocalDateTime.of(2022, 7, 23, 0, 0);
	...
	public static List<Statement> lessThanNinetyDays(List<Statement> statements) {
		...
		List<Statement> resultado = new ArrayList<>();

		for (Statement statement : statements) {
			if (statement.getDate().isAfter(HOJE.minusDays(90)))
				resultado.add(statement);
		}
	...
	public static List<Statement> lessThanFourtyFiveDays(List<Statement> statements) {
		...
		for (Statement statement : statements) {
			if (statement.getDate().isAfter(HOJE.minusDays(45)))
				resultado.add(statement);
		}
	...
	/**
	 * Métodod responsável por filtrar os lançamentos entre um período menor que 90
	 * dias.
	 * 
	 * @param statements
	 * @return List<Statement2>
	...
	public static List<Statement> betweenDays(List<Statement> statements, LocalDateTime from, LocalDateTime to)
		...
		for (Statement statement : statements) {
			if (statement.getDate().isAfter(from.minusDays(1)) && statement.getDate().isBefore(to.plusDays(1)))
				resultado.add(statement);
		}
	...
	/**
	 * Este método representa um teste unitário que valida o comportamento do método
	 * inTheYear.
	 * 
	 * @param statements
	 */
	...
	private static void given_listOfStatements_when_period_are_grether_than_ninety_days_then_throw_exception(
			List<Statement> statements) {
		try {
			LocalDateTime from = HOJE.minus(95l, ChronoUnit.DAYS);
			LocalDateTime to = HOJE;
	...
	private static void given_listOfStatements_when_period_are_less_than_ninety_days_then_return_statements(
		...
		LocalDateTime from = HOJE.minus(5l, ChronoUnit.DAYS);
		LocalDateTime to = HOJE;
	...
	/**
	 * Este método é usado dentro dos teste unitários para exibir uma mensagem de
	 * sucesso ou de fallha, caso o resultado não seja igual ao esperado.
	 * 
	 * @param result
	 * @param expected
	 */
 ...
 * 
 *         É importante que essa classe tenha os métodos equals e hashcode para
 *         que na comparação do
